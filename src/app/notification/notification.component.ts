...
export class NotificationComponent implements OnInit {
  ...
  addComponent(componentClass: Type<any>, config) {
    ...
    component.instance.divStyle = {
      'background-color': config.status === 'error' ? '#f2dede' : config.status === 'success' ? '#d4888d' : config.status === 'info' ? '#d9edf7' : '#fcf8e3'
    };
    component.instance.style = {
      ...
      color: '#000',
      'background-color': config.status === 'error' ? '#fe4949' : config.status === 'success' ? '#d4888d' : config.status === 'info' ? '#1d9fb4' : '#daae00',
